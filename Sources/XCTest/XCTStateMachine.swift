...
O: DSLCompatible & Equatable {
    ...
    public func assert(
        ...
        for state in states {
            let receivedCommands = self.stateMachine.command(for: state)
            guard receivedCommands.contains(expectedOutput) else {
                onFail(
                    """
                    The assertion failed for state \(state):
                    expected output: \(expectedOutput),
                    received output: \(String(describing: receivedCommands))
                    """
                )
                return self
            }
    ...
    public func assertNoOutput(
        ...
        for state in states {
            ...
            if !receivedCommands.isEmpty {
                ...
                The assertion failed for state \(state):
                expected no output,
                received output: \(receivedCommands)
                """
                )
                return self
            }
